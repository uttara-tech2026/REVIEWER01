...
- The reviewer is sent one item at a time with **✅ Send** / **❌ Reject**
  buttons:
  - **Send** → delivered to every enabled destination (or back into the
    owner's chat if none are set), with the caption **forced to the
    queue's name** — this always applies to review-approved sends,
    regardless of `/togglequeuecaption`.
  - **Reject** → the item is dropped from the queue and deleted (the
