...
HELP_TEXT = (
    ...
    "Reviewer taps ✅ Send (goes to your destinations, caption = queue\n"
    "  name) or ❌ Reject (deleted). Bot auto-advances to the next item.\n\n"
    "OTHER\n"
...
async def _push_next_review_item(bot, owner_chat_id: int, chat: dict):
    ...
    buttons = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ Send", callback_data=f"rev:{owner_chat_id}:send:{token}"),
        InlineKeyboardButton("❌ Reject", callback_data=f"rev:{owner_chat_id}:reject:{token}"),
    ]])
